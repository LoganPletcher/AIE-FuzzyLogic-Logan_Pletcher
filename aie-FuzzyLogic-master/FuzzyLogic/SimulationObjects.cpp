...
AgentPredator::AgentPredator(glm::vec2 position) : BaseAgent(position, glm::vec4(1, 0, 1, 1), 20)
{
	type = PREDATOR_AI;
	maxSpeed = 50;
}
...
float Agent::checkSleepDesirable()
{
	...
	//is it a desirable action?
	float desirableValue = Fuzzy::OR(Fuzzy::NOT(caveFar), awake);
	//is in undesirable?  In this case if we are full it's undesirable
...
float Agent::checkDrinkingDesirable()
{
	...
	///My code
	//is this a very desirable action?
	float veryDesirableValue = Fuzzy::OR(Fuzzy::AND(waterRangeClose, veryThirsty), weekFromThirst);
	//is it a desirable action?
	float desirableValue = Fuzzy::OR(Fuzzy::AND(waterRangeClose, thirsty), notThirsty);
	//is in undesirable?  In this case if we are full it's undesirable
...
float Agent::checkPredatorDesirable()
{
	float desire = 0; //this is the return value which will be how much
	//our AI desires to avoid the predator
	float predatorRange = findNearestResource(PREDATOR_AI);
	//how far is the predator
	//how close we are to the predator...
	...
	//is this a very desirable action?
	float veryDesirableValue = Fuzzy::OR(predatorRangeClose, Fuzzy::NOT(predatorRangeFar));
	//is it a desirable action?
	float desirableValue = Fuzzy::AND(predatorRangeMedium, Fuzzy::NOT(predatorRangeFar));
	//is it undesirable? In this case if the predator is far away
...
glm::vec2 Agent::avoidPredator(float desirability, float delta)
{
	glm::vec2 velocity = findResourceVector(PREDATOR_AI) * delta * (1 + desirability) * maxSpeed;
	return -velocity;
}

glm::vec2 AgentPredator::gotoAgent(float delta)
{
	glm::vec2 velocity = findResourceVector(SIMPLE_AI) * delta * maxSpeed;
	return velocity;
}
...
void AgentPredator::update(float delta)
{
	glm::vec2 velocity = gotoAgent(delta);
	position += velocity;
}

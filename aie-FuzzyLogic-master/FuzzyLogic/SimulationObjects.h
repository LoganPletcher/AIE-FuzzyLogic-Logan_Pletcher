...
public:
	...
	glm::vec2 avoidPredator(float desirability,float delta);
};

class AgentPredator : public BaseAgent
{
public:
	float maxSpeed;
	virtual void update(float delta);
	virtual void draw();
	AgentPredator(glm::vec2 position);
	glm::vec2 gotoAgent(float delta);
};

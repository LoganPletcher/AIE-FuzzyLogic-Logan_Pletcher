...
{
	int randomWidth;
	int lowestWidth = 100, highestWidth = 1000;
	int widthRange = (highestWidth - lowestWidth) + 1;

	int randomHeight;
	int lowestHeight = 100, highestHeight = 700;
	int heightRange = (highestHeight - lowestHeight) + 1;

	//set up simulation world.
	worldController = new WorldController();
	randomWidth = lowestWidth + int(widthRange * (rand() / (RAND_MAX + 1.0)));
	randomHeight = lowestHeight + int(heightRange * (rand() / (RAND_MAX + 1.0)));
	Agent* simpleAI = new Agent(glm::vec2(randomWidth, randomHeight));
	worldController->addObject(simpleAI);
	randomWidth = lowestWidth + int(widthRange * (rand() / (RAND_MAX + 1.0)));
	randomHeight = lowestHeight + int(heightRange * (rand() / (RAND_MAX + 1.0)));
	AgentPredator* predatorAI = new AgentPredator(glm::vec2(randomWidth, randomHeight));
	worldController->addObject(predatorAI);
	randomWidth = lowestWidth + int(widthRange * (rand() / (RAND_MAX + 1.0)));
	randomHeight = lowestHeight + int(heightRange * (rand() / (RAND_MAX + 1.0)));
	Water* water1 = new Water(glm::vec2(randomWidth, randomHeight));
	worldController->addObject(water1);
	randomWidth = lowestWidth + int(widthRange * (rand() / (RAND_MAX + 1.0)));
	randomHeight = lowestHeight + int(heightRange * (rand() / (RAND_MAX + 1.0)));
	Cave* cave = new Cave(glm::vec2(randomWidth, randomHeight));
	worldController->addObject(cave);
	randomWidth = lowestWidth + int(widthRange * (rand() / (RAND_MAX + 1.0)));
	randomHeight = lowestHeight + int(heightRange * (rand() / (RAND_MAX + 1.0)));
	Food* food = new Food(glm::vec2(randomWidth, randomHeight));
